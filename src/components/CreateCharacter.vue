<template>
  <v-container id="createcharacter">
    ...
    <!--<Dialog v-if="creationError" header="Error" :body="errorMessage" callback="() => { creationError = false; }" />
    <Dialog v-if="creationSuccess" header="Created" body="Character created" callback="() => { CallbackSuccess(); }" />-->
  </v-container>
</template>
<script lang="ts">
